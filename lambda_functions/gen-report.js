...
export const handler = async (event) => {
    ...
    let getReservations = (restaurantID, date) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT date,time,table_num,guests, email, rid, confirmation_code FROM tables4u.Reservations WHERE restaurant = ? AND date =?;", [restaurantID, date], (error, rows) => {
                if (error) { return reject(error); }
                return resolve(rows);
            })
        })
    ...
    let ts = await getTables(event.uid)
    if (ts.length == 0) {
        pool.end();
        return {
            statusCode: 400,
    ...
    let schedules = await getSchedules(event.uid)
    let schedule = 0
    switch (day) {
        case 0:
    ...
    let reservations = await getReservations(event.uid, event.date)
    //Total Seats
    let totalSeats = 0
    ...
    for (let i = hours.opening; i < hours.closing; i += 100) {
        ...
        utilization.push(Math.round(100 * totalGuests / totalSeats))
        reserved.push(line)
        availability.push(Math.round(100*(ts.length-resTables)/ts.length))
    }
    pool.end();
    return {
        statusCode: 200,
